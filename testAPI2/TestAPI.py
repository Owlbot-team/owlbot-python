...
def MoveCameraOwl():
 ...
 xValue = 147
 zValue = 66
 response = requests.get(url, params={'value': str(value), 'xyval': str(xValue), 'zval': str(zValue), 'token': token})
...
Camera_OWL = tk.Button(frame,
                  text="OWL",
                   command=MoveCameraOwl)
Camera_OWL.pack(side=tk.RIGHT)

Camera_LIGHT = tk.Button(frame,
                   text="Howard",
                   command=MoveCameraLight)
